...
docker run --user "$(id -u):$(id -g)" -d --name sales-browser-audit-web --network sales-browser-audit-net --read-only --cap-drop ALL --security-opt no-new-privileges --memory 512m --cpus 1 --tmpfs /tmp --env-file "$base/test.env" --mount "type=bind,source=$base/app,target=/app" --workdir /app --entrypoint php nextcloud:29-apache artisan serve --host=0.0.0.0 --port=8000
docker inspect --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}:8000' sales-browser-audit-web > "$base/address.txt"
